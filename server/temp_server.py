...
users_db='users.db'
#db for bids looks like this: <buy_currency>_<sell_currency>.db
PORT=8091
...
def append_to_trades(Bid):
    file=Bid['buy_currency']+'_'+Bid['sell_currency']+'_trades.db'
    db=fs_load(file)
    db.append(Bid)
    fs_save(file, db)
...
#    return out.format('')
#    return """0,1;"1FfmbHfnpaZjKFvyi1okTjJJusN455paPH","1676NCiezxodBpM5j1Zafnuxuaeuz2vfPR";1.0,1.0;'Silver - Unchained','Silver - Unchained'"""
    return '''var everything = [
  [0, "1FfmbHfnpaZjKFvyi1okTjJJusN455paPH", 1.0, 'Silver - Unchained'],
  [1, "1676NCiezxodBpM5j1Zafnuxuaeuz2vfPR", 1.0, 'Silver - Unchained'],
  ];'''
